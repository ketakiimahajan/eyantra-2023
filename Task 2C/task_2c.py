...
# Team ID:			[ 1880 ]
# Author List:		[ ketaki mahajan, rena shoby, sammod date, pratik panchal ]
# Filename:			task_2c.py
...
def event_identification(arena):        # NOTE: You can tweak this function in case you need to give more inputs 
    ...
    _, binary_arena = cv.threshold(gray_arena, 200, 255, cv.THRESH_BINARY)

    # Find contours in the binary image
    contours, _ = cv.findContours(binary_arena, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE)

    # Initialize an empty list to store identified events
    event_list = []
...
def classify_event(image):
    ...
    ADD YOUR CODE HERE
    '''
    new_model = load_model('imageclassifier_task2B.h5')
    image = tf.convert_to_tensor(image, dtype=tf.float32)
    resize = tf.image.resize(image, (256,256))
    
    yhat = new_model.predict(np.expand_dims(resize/255, 0))
    
    event_mapping = {0: combat, 4: rehab, 3: military_vehicles, 2: fire, 1: destroyed_building}
    event_index = np.argmax(yhat)
    
    event = event_mapping[event_index]
